...
# =========================================
# FULL ELO + AUTO MATCH SYSTEM (ADDED)
# =========================================

def add_win(user_id, elo_gain=25):
    users[user_id]["wins"] += 1
    users[user_id]["elo"] += elo_gain
    users[user_id]["level"] = users[user_id]["elo"] // 100 + 1


def add_loss(user_id, elo_loss=15):
    users[user_id]["loses"] += 1
    users[user_id]["elo"] = max(0, users[user_id]["elo"] - elo_loss)
    users[user_id]["level"] = users[user_id]["elo"] // 100 + 1


def team_strength(team):
    return sum(users[p["id"]]["elo"] for p in team)


def auto_finish_match(match_id):
...
🏆 Winners:
{', '.join([x['name'] for x in winners])}

💀 Losers:
{', '.join([x['name'] for x in losers])}
